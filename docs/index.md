...
> **Note:** If both the library and the extension are installed, the extension takes precedence. Operations that rely on the extension (Eigen decomposition, SVD, pseudoinverse) throw `Tensor\Exceptions\NotImplemented` in the pure-PHP library.
